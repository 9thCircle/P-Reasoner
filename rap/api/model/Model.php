...
class Model extends RDFObject
{
    ...
    public function load($filename, $type = NULL, $stream = FALSE)
    {
        if ((isset($type)) && ($type ==='n3') OR ($type ==='nt')) {
            ...
            if (isset($suffix[1]) && ($ext === 'n3' OR $ext === 'nt')){
                ...
                include_once RDFAPI_INCLUDE_DIR.PACKAGE_SYNTAX_N3;
                $parser = new N3Parser();
            }elseif (isset($suffix[1]) && ($ext === 'htm' OR $ext === 'html')){
                    include_once RDFAPI_INCLUDE_DIR.PACKAGE_SYNTAX_GRDDL;
                    $parser = new GRDDLParser();
            }else{
                // Import Package Syntax
    ...
    * @access	private
    */
    protected function getUniqueResourceURI($prefix = FALSE)
    {
        static $bNodeCount;
        if(!$bNodeCount)
            $bNodeCount = 0;
    ...
    * @access	public
    */
    public static function getResModel()
    {
    ...
    * @access	public
    */
    public static function getOntModel($vocabulary)
    {
    ...
    *
    */
    public static function getMemModelByRDQL($queryString, $closure = FALSE)
    {
