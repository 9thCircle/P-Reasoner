...
class RdfParser extends RDFObject
{
	private $rdf_parser;
	private $model;
	...
	/**
	 * \todo	This should be RDFElement::__construct()
	 * @access	private
	 */
	...
	/**
	  *	\todo	This should be a RDFElement::__clone()
	  * @param string $source
	...
	/**
	 * \todo	This should be RDFElement::clear()
	 * @param string &$e
	 * @access	private
	 */
...
/*
   ...
   * @access	private
*/
	private function _parse_uri($uri, &$scheme, &$authority, &$path, &$query, &$fragment)
	{
...
/**
   ...
   * @access	private
*/
function _report_statement( $subject_type, $subject, $predicate, $object_type,  $object, $xml_lang, $bag_id, $statements, $statement_id, $datatype)
{
	...
	// call add statement
	$this->add_statement_to_model($subject_type, $subject, $predicate, $object_type, $object, $xml_lang, $datatype);
...
function _handle_property_attributes($subject_type, $subject, $attributes, $xml_lang, $bag_id, $statements)
{
    ...
    for($i = 0; isset($attributes[$i]); $i += 2 ) {
        ...
        if (RDF_NAMESPACE_URI === $attribute_namespace_uri) {
            if ($this->_is_rdf_property_attribute_literal($attribute_local_name)) {
				...
				$this->_report_statement(
                    ...
                    '',
					'' );
            } elseif (  $attribute_local_name !== RDF_ABOUT
        ...
        } elseif (XML_NAMESPACE_URI === $attribute_namespace_uri) {
...
function _handle_resource_element($namespace_uri, $local_name, $attributes, $parent)
{
    ...
    for($i = 0; isset($attributes[$i]); $i += 2) {
        ...
        } elseif($attribute_namespace_uri === XML_NAMESPACE_URI) {
...
function _handle_property_element(&$namespace_uri, &$local_name, &$attributes)
{
    ...
    for( $i = 0; isset($attributes[$i]); $i += 2 )
    {
            ...
            || (  $attribute_namespace_uri === RDF_NAMESPACE_URI ) )
        {
            ...
            } elseif($attribute_local_name === RDF_PARSE_TYPE) {
                $parse_type = $attribute_value;
            } elseif($attribute_local_name === RDF_RESOURCE) {
                $resource = $attribute_value;
            } elseif($attribute_local_name === RDF_NODEID) {
				$node_id = $attribute_value;
            } elseif($attribute_local_name === RDF_BAG_ID) {
                $bag_id = $attribute_value;
            } elseif($attribute_local_name === RDF_DATATYPE) {
				$datatype = $attribute_value;
    ...
    } elseif( $resource !== NULL || $bag_id || $this->rdf_parser['top']['has_property_attributes'] ) {
        if( $resource !== NULL ) {
...
{
    $state = $this->rdf_parser['top']['state'];
	if ($state === IN_XML) {
		$this->_handle_xml_end_element($name);
    } elseif ($state === IN_PROPERTY_UNKNOWN_OBJECT || $state === IN_PROPERTY_LITERAL) {
        $this->_end_literal_property( );
    } elseif ($state === IN_PROPERTY_PARSE_TYPE_RESOURCE) {
        $this->_pop_element();
    } elseif ($state === IN_PROPERTY_PARSE_TYPE_LITERAL) {
		$this->rdf_parser['top']['data'] = $this->rdf_parser['xml_literal']['buffer'];
		$this->_end_literal_property();
		$this->rdf_parser['xml_literal']['buffer'] = '';
	} elseif ($state === IN_PROPERTY_PARSE_TYPE_COLLECTION) {
...
function _character_data_handler($parser, $s)
{
    $len = strlen($s);
    $state = $this->rdf_parser['top']['state'];
	if ($state === IN_PROPERTY_LITERAL || $state === IN_PROPERTY_UNKNOWN_OBJECT) {
        $i = 0;
	...
	} elseif ($state === IN_PROPERTY_PARSE_TYPE_LITERAL || $state === IN_XML) {
        $this->rdf_parser['xml_literal']['buffer'] .= $s;
	}
	/*
		Ignored values:
			IN_TOP_LEVEL, IN_RDF, IN_DESCRIPTION, IN_PROPERTY_RESOURCE, IN_PROPERTY_EMPTY_RESOURCE, 
			IN_PROPERTY_PARSE_TYPE_RESOURCE, IN_UNKNOWN
	*/
}
	...
	$datatype )
{
	...
	//create subject
	if ($subject_type === RDF_SUBJECT_TYPE_BNODE) {
		$objsub = new RDFBlankNode($subject);
	} else {
		$objsub = new RDFResource($subject);
	}
	
	// create object
	if ($object_type === RDF_OBJECT_TYPE_RESOURCE || $object_type === RDF_OBJECT_TYPE_BNODE) {
		if ($object_type === RDF_OBJECT_TYPE_BNODE) {
			$objobj = new RDFBlankNode($object);
		} else {
			$objobj = new RDFResource($object);
		}
	} else {
		$objobj = new RDFLiteral($object);
		if ($datatype !== '') {
	...
	// create statement
	$statement = new Statement($objsub, new RDFResource($predicate), $objobj);
...
function & generateModel($base, $rdfBaseURI = FALSE, $model = FALSE)
{
	...
	if (substr(ltrim($base), 0, 1) !== '<') {
		...
		while($done === FALSE) {
			if ( ! $this->rdf_parse(fread($input, 512), $done = feof($input) ) ) {
		...
		$this->rdf_set_base( $rdfBaseURI === FALSE ? NULL : $rdfBaseURI );
...
function rdf_set_base($base)
{
	...
	$this->rdf_parser['normalized_base_uri'] = ($c === '#' || $c === ':' || $c === '/' || $c === "\\") !== TRUE ?
		$base . '#' :
		$base;
